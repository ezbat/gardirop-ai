...
import {
  ArrowLeft, Loader2, Send, Headphones, User,
} from 'lucide-react'

const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  open:             { label: 'Offen',          color: '#D97706', bg: '#FEF3C7' },
  in_progress:      { label: 'In Bearbeitung', color: '#2563EB', bg: '#DBEAFE' },
  waiting_customer: { label: 'Wartet',         color: '#D97706', bg: '#FEF3C7' },
  waiting_seller:   { label: 'Antwort nötig',  color: '#DC2626', bg: '#FEE2E2' },
...
export default function SellerTicketDetailPage() {
  ...
  async function loadTicket() {
    try {
      const res = await fetch(`/api/seller/support/${ticketId}`)
      if (!res.ok) throw new Error()
      const data = await res.json()
      setTicket(data.ticket)
      setMessages(data.messages ?? [])
    } catch {
      console.error('[seller/support detail] load failed')
    } finally {
      setLoading(false)
    }
  }
              ...
              <div key={m.id} className="px-5 py-4">
                <div className="flex items-center gap-2 mb-2">
                  <div
                    ...
                    style={{ background: isSeller ? '#FEF3C7' : '#DBEAFE' }}
                  >
                    {isSeller
                  ...
                  <span className="text-xs font-semibold" style={{ color: isSeller ? '#D97706' : '#2563EB' }}>
                    {isSeller ? 'Du' : 'Support-Team'}
                  </span>
