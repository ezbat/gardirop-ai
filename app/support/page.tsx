...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  ...
  waiting_customer: { label: 'Antwort nötig',    color: '#DC2626', bg: '#FEE2E2', icon: AlertCircle },
  waiting_seller:   { label: 'Wartet',           color: '#D97706', bg: '#FEF3C7', icon: Clock },
...
export default function CustomerSupportPage() {
  ...
  async function loadTickets() {
    try {
      ...
      console.error('[support] load failed')
    } finally {
      setLoading(false)
    }
  }
        ...
        {tickets.length === 0 ? (
          <div className="text-center py-16 rounded-2xl" style={{ background: '#FFF', border: '1px solid #E5E5E5' }}>
            ...
            <p className="text-sm mt-1" style={{ color: '#BBB' }}>
              Erstelle ein Ticket, wenn du Hilfe benötigst.
            </p>
          </div>
        ) : (
