...
export default function PostDetailModal({ post, isOpen, onClose, onLikeToggle, onDelete }: PostDetailModalProps) {
  ...
  const loadComments = async () => {
    ...
    setLoading(true)
    try {
      const { data: commentsData, error } = await supabase.from('comments').select('*').eq('post_id', post.id).order('created_at', { ascending: true })
      if (error) throw error
      const commentsWithUsers = await Promise.all((commentsData || []).map(async (comment) => {
        const { data: userData } = await supabase.from('users').select('id, name, avatar_url').eq('id', comment.user_id).single()
        return { ...comment, user: userData || { id: comment.user_id, name: 'Unknown', avatar_url: null } }
      }))
  ...
  const handleSubmitComment = async () => {
    ...
    setSubmitting(true)
    try {
      const { error } = await supabase.from('comments').insert({ post_id: post.id, user_id: userId, content: newComment.trim() })
      if (error) throw error
      if (post.user_id !== userId) {
        await createNotification(post.user_id, userId, 'comment', 'gönderine yorum yaptı')
      }
      setNewComment("")
      await loadComments()
      await supabase.from('posts').update({ comments_count: comments.length + 1 }).eq('id', post.id)
    } catch (error) {
  ...
  const handleLike = async () => {
    ...
    try {
      if (currentlyLiked) {
        ...
        if (post.user_id !== userId) {
          await createNotification(post.user_id, userId, 'like', 'gönderini beğendi')
        }
      }
      if (onLikeToggle) {
        ...
        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-4xl h-[90vh] glass border border-border rounded-2xl overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="md:w-1/2 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {loading ? (<div className="flex justify-center py-8"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>) : comments.length === 0 ? (<div className="text-center py-8 text-muted-foreground"><p>Henüz yorum yok. İlk yorumu sen yap!</p></div>) : (comments.map((comment) => (<div key={comment.id} className="flex gap-3"><div className="w-8 h-8 rounded-full overflow-hidden bg-primary/10 flex-shrink-0">{comment.user?.avatar_url ? (<img src={comment.user.avatar_url} alt={comment.user.name} className="w-full h-full object-cover" />) : (<div className="w-full h-full flex items-center justify-center text-xs">{comment.user?.name?.[0]?.toUpperCase() || 'U'}</div>)}</div><div className="flex-1"><p className="text-sm"><span className="font-bold mr-2">{comment.user?.name}</span>{comment.content}</p><p className="text-xs text-muted-foreground mt-1">{new Date(comment.created_at).toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' })}</p></div></div>)))}
            ...
            <div className="border-t border-border">
              ...
              <div className="p-4 pt-0 flex gap-2">
                <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} onKeyPress={handleKeyPress} placeholder="Yorum yaz..." className="flex-1 px-4 py-2 glass border border-border rounded-xl outline-none focus:border-primary" disabled={submitting || !userId} />
